...
: X_Pos (AsConfig::Border_X_Offset), X_Step (AsConfig::Global_Scale * 2), Platform_State (EPS_Missing), Inner_Width (Normal_Platform_Inner_Width), Rolling_Step (0), 
  ...
  Highlight_Color (255, 255, 255), Platform_Circle_Color (1, 255, 1), Platform_Inner_Color (255, 80, 190)
{
...
{// Корректируем позицию при отражении от платформы.
    ...
    // 1. Проверка отражений от боковых шариков.
    ...
    // 2. Проверка отражения от центральной части платформы.
    if (ball-> Is_Moving_Up ())
        inner_y = inner_low_y; // От нижней грани платформы.
    else
        inner_y = inner_top_y; // От верхней грани платформы.
...
//------------------------------------------------------------------------------------------------------------------------
bool AsPlatform::Move (bool to_left)
{
...
{// Отрисовка платформы в нормальном состоянии.
    ...
    inner_rect.bottom = (y + 1 + 5) * AsConfig::Global_Scale - 1;
...
{// Отрисовка платформы в расплавленном состоянии.
    ...
    for (i = 0; i < Normal_Platform_Image_Width; i++)
    {
        ...
        // Рисование последовательности вертикальных штрихов разного цвета (согласно прообразу, сохранённому в Normal_Platform_Image)
        ...
        // Стирание фоном пиксели над штрихом.
        y = Meltdown_Platform_Y_Pos [i];
        MoveToEx (hdc, x, y, 0);
    ...
    if (moved_columns_count == 0)
        Platform_State = EPS_Missing; // Платформа сдвинулась за пределы окна.
}
//------------------------------------------------------------------------------------------------------------------------
void AsPlatform::Draw_Roll_In_State (HDC hdc, RECT &paint_area)
{// Отрисовка выкатывающейся платформы.
    ...
    alpha = -AsConfig::PI_2 / (double) Max_Rolling_Step * (double) Rolling_Step;
...
{// Отрисовка расширяющейся платформы.
...
bool AsPlatform::Reflect_On_Circle (double next_x_pos, double next_y_pos, double platform_ball_x_offset, ABall *ball)
{
    ...
    {// Мячик коснулся бокового шарика.
        ...
        if (related_ball_direction > AsConfig::PI_2)
            related_ball_direction -= AsConfig::PI_2;
...
{// Вычисляем длинну очередного вертикального штриха.
