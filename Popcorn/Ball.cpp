...
{// Функция проверяет пересечение горизонтального отрезка (проходящего через left_x до right_x через y) с окружностью радиусом radius.
...
void ABall::Set_For_Test ()
{
    ...
    //Set_State (EBS_Normal, 80 + Test_Iteration, 195);
...
void ABall::Set_State (EBall_State new_state, double x_pos, double y_pos)
{
    ...
    case EBS_On_Parachute:
       AsConfig::Throw (); // Для постановки на паращют нужно вызвать метод Set_On_Parachute ().
       break;
    ...
    case EBS_Teleporting:
       if (! (Ball_State == EBS_Normal || Ball_State == EBS_On_Parachute || Ball_State == EBS_Teleporting))
          AsConfig::Throw (); // Только из этих состояний можно телепортировать мячик.
...
{
    while (new_direction > AsConfig::PI_2)
        new_direction -= AsConfig::PI_2;
...
void ABall::Draw_Parachute (HDC hdc, RECT &paint_area)
{
   ...
   int arc_x;
   int line_y = Parachute_Rect.top + dome_height;
   ...
   Chord (hdc, Parachute_Rect.left, Parachute_Rect.top, Parachute_Rect.right - 1, Parachute_Rect.bottom - 1,
         Parachute_Rect.right, line_y - 1, Parachute_Rect.left, line_y - 1);
   ...
   MoveToEx (hdc, Parachute_Rect.right - 4 * scale, line_y, 0);
   LineTo (hdc, ball_center_x, ball_center_y);
