...
void AFalling_Letter::Set_Brick_Letter_Colors (bool is_switch_color, const AColor **front_color, const AColor **back_color)
{
    if (is_switch_color)
    {
        *front_color = &AsConfig::Purpule_Color;
        *back_color = &AsConfig::Whiteblue_Color;
    }
    else
    {
        *front_color = &AsConfig::Whiteblue_Color;
        *back_color = &AsConfig::Purpule_Color;
    }
}
//------------------------------------------------------------------------------------------------------------------------
...
{// Вывод падающей буквы.
    ...
    if (!(Brick_Type == EBT_Whiteblue || Brick_Type == EBT_Purpule))
        return; // Падающие буквы могут быть только от кирпичей такого типа.

    // Корректируем шаг вращени я угол поворота.
    Rotation_Step = Rotation_Step % Max_Rotation_Step;

    if (Rotation_Step < 8)
        rotation_angle = AsConfig::PI_2 / Max_Rotation_Step * (double) Rotation_Step;
    else
        rotation_angle = AsConfig::PI_2 / Max_Rotation_Step * (double) (8 - Rotation_Step);

    if (Rotation_Step > 4 && Rotation_Step <= 12)
    {
        if (Brick_Type == EBT_Whiteblue)
            switch_color = true;
        else
            switch_color = false;
    }
